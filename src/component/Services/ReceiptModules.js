    ...
    loadReceiptData(saleGroupId, () => {
      setTimeout(() => {
        if (receiptRef.current && receiptData && receiptData.group.id === saleGroupId) {
          ...
          alert('Try printing again by clicking the button.');
        }
  ...
  return (
    <div className="w-full p-6 sm:p-6 space-y-6 dark:bg-gray-900 dark:text-white">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-white dark:text-white">.</h2>
        <div className="flex gap-4">
          ...
          >
            {printLoading ? 'Loading...' : 'Print Latest'}
          </button>
        </div>
      </div>
