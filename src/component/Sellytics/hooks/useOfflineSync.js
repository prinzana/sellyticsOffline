...
export default function useOfflineSync(onSyncComplete) {
  ...
  const updateQueueCount = useCallback(async () => {
    ...
    const count = await offlineCache.getGlobalPendingCount(currentStoreId);
    setQueueCount(count);
  }, [currentStoreId]);
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      const sales = await offlineCache.getPendingSales(currentStoreId);
    ...
    async (item) => {
      ...
      try {
        ...
        if (entity_type === 'stock_transfers' && operation === 'create') {
          const { syncTransfer } = await import('../StockTransfer/stockTransferSyncHandler');
  ...
  const clearQueue = useCallback(async () => {
    // OLD: await offlineCache.clearSyncQueue(currentStoreId);
    // NEW: Delete actual pending sales data, not just queue
    if (offlineCache.deleteAllPendingSales) {
      await offlineCache.deleteAllPendingSales(currentStoreId);
    } else {
      console.warn('deleteAllPendingSales not found on offlineCache');
    }

    await updateQueueCount();
    toast.success('All pending sales deleted');
  }, [currentStoreId, updateQueueCount]);
