...
export function useOfflineSync(storeId, onSyncComplete) {
  ...
  const syncQueueItem = useCallback(async (item) => {
    if (!supabaseRef.current) throw new Error('Supabase not available');
    ...
    case 'update': {
  const { id, updates, addedQty = 0 } = data; // addedQty for offline qty increments
  ...
  // 3️⃣ Increment inventory instead of replacing
  const newQty = (inv?.available_qty || 0) + (updates.purchase_qty || 0) + addedQty;
...
default:
  ...
  const syncNow = useCallback(async () => {
    if (!isOnline || syncInProgress.current || !supabaseRef.current) {
