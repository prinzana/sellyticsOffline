...
export default function RestockTable({ recommendations, onDelete }) {
  return (
    <div className="space-y-4">
      {recommendations.map((rec, i) => {
        const forecastDate = new Date(rec.month).toLocaleDateString();
        const badgeClasses = rec.recommendation.includes("Restock")
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              ...
              <div className="flex flex-col sm:flex-row items-start gap-4 flex-1 min-w-0 pr-12 sm:pr-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2 space-y-1 text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-700 dark:text-gray-300">Quantity Sold:</span>
                      <span>{rec.quantity_sold} units</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-700 dark:text-gray-300">Recommendation:</span>
                      <span className={badgeClasses}>{rec.recommendation}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="items-center gap-4 hidden sm:flex">
                <ActionsMenu onDelete={() => onDelete(rec.id)} />
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 sm:hidden text-center text-sm text-slate-500">
              Tap for details • Use ⋮ for actions
            </div>
          </motion.div>
        );
      })}
