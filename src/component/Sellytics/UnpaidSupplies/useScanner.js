/**
 * SwiftCheckout - Scanner Hook for UnpaidSupplies
...
export default function useScanner({ onScanSuccess }) {
  ...
  const [targetEntryIndex, setTargetEntryIndex] = useState(null);
  const [targetDeviceIndex, setTargetDeviceIndex] = useState(null);
  ...
  const processScannedCode = useCallback(async (barcode) => {
    ...
    try {
      const result = await onScanSuccess(code, targetEntryIndex, targetDeviceIndex);
  ...
  }, [onScanSuccess, targetEntryIndex, targetDeviceIndex, continuousScan, closeScanner]);

  
  // Open scanner
  const openScanner = useCallback((mode = 'external', entryIndex = null, deviceIndex = null) => {
    setScannerMode(mode);
    setShowScanner(true);
    setManualInput('');
    setTargetEntryIndex(entryIndex);
