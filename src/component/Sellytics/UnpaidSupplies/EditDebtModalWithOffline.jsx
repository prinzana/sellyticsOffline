...
export default function EditDebtModal({ initialData, onClose, onSuccess }) {
    ...
    const handleAddPayment = (entryIndex, payment) => {
        ...
        if (!updatedEntries[entryIndex].payments) {
            ...
            try {
                if (isOnline) {
                    ...
                    if (cachedCustomers.length === 0 || cachedProducts.length === 0) {
                        addNotification({
                            type: 'warning',
                            message: 'Limited data available offline. Some features may be restricted.'
                        });
                    }
                }
            } catch (error) {
                ...
                >
                    ...
                    {isLoadingData ? (
                        ...
                        <form onSubmit={handleSubmit} ref={formRef} className="flex-1 overflow-y-auto p-5 space-y-6">
                            {/* Offline Notice */}
                            {!isOnline && (customers.length === 0 || products.length === 0) && (
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-amber-700 dark:text-amber-400">
                                                {customers.length === 0 && 'No customers cached. '}
                                                {products.length === 0 && 'No products cached. '}
                                                Connect to the internet to access full data.
                                            </p>
                                        </div>
                                    </div>
                                </div>
