/**
 * Receipt Preview Component - Optimized for thermal printing and 100+ line items
 * FIXED: Edited customer name, phone, warranty now appear correctly
 * Uses receipt fields first (edited values), with safe fallbacks
 */
...
const ReceiptPreview = forwardRef(({ store, receipt, saleGroup, productGroups, styles }, ref) => {
  const { formatPrice = (amount) => `$${Number(amount || 0).toLocaleString()}`} = useCurrency();

  // Use receipt values first (these are updated when editing)
  ...
  // Customer details - prioritize edited receipt values
  ...
  return (
    <div ref={ref} className="bg-white w-full max-w-[80mm] mx-auto text-xs" style={{ fontFamily: 'monospace' }}>
      {/* Header */}
      <div 
        className="text-center p-4 mb-3"
        style={{ 
          backgroundColor: styles.headerBgColor || '#f8fafc', 
          color: styles.headerTextColor || '#000000' 
        }}
      >
      ...
      {/* Items - Optimized for many line items */}
      ...
      {/* Customer Info - NOW SHOWS EDITED VALUES */}
      {hasCustomerInfo && (
        <>
